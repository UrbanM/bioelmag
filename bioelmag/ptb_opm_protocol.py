...
def create_mne_raw(sens_info, meas_info, data_path, channel_factor=(10 ** -9), sens_hol_path="", subject_dir="", opm_trans_path="", geom_name=""):
    import mne
    import numpy as np
    ...
    info = mne.create_info(ch_names=sens_info["sens_names"], sfreq=sfreq, ch_types=sens_info["sens_type"])
    ...
    for i, j in enumerate(info.ch_names):
        if "oMEG" in j:
            ...
            data[sens_info['sens_datapos'][sens_hold_idx]] = data[sens_info['sens_datapos'][sens_hold_idx]] * channel_factor
...
#functions bellow are not crucial
